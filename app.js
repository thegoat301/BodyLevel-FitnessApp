...
/* badges */
const BADGES = {
  motivated:{name:"🔥 Motivé",condition:u=>u.streak>=3,desc:"Avoir une streak ≥ 3"},
  regular:{name:"💪 Régulier",condition:u=>u.streak>=7,desc:"Avoir une streak ≥ 7"},
  xp500:{name:"⚡ 500 XP",condition:u=>u.xp>=500,desc:"XP ≥ 500"},
  xp1000:{name:"🏆 1000 XP",condition:u=>u.xp>=1000,desc:"XP ≥ 1000"},
  discipline:{name:"🧠 Discipline",condition:u=>u.history.length>=20,desc:"Avoir ≥ 20 séances"}
};
...
function checkBadges(){
  let users=getUsers(); let user=users[currentUser];
...
function loadExercises(){
  ...
  allowed.forEach(ex=>{ let option=document.createElement("option"); option.value=ex; option.textContent=ex; select.appendChild(option); });
}
...
document.getElementById("finishWorkoutBtn").onclick = () => {
  ...
  if(data.dailySessions[today]>=maxSessions){ alert("Limite de séances atteinte !"); return; }
  data.dailySessions[today]++;

  let gainedXP = 0, totalReps = 0;
  workout.forEach(w=>{
    gainedXP += w.reps * xpValues[w.ex]; totalReps += w.reps;
...
document.getElementById("navProfile").onclick = () => {
  ...
  document.getElementById("profileName")?.remove(); // remove old if exists
  let h4 = document.createElement("h4"); h4.id="profileName"; h4.innerText="Nom : "+currentUser;
  document.getElementById("profileModal").querySelector(".modal-content").prepend(h4);
...
document.getElementById("navBadges").onclick = () => {
  ...
  Object.keys(BADGES).forEach(key=>{
    ...
    li.innerText=(data.badges.includes(key)?BADGES[key].name+" ✔":"🔒 "+BADGES[key].name)+" - "+BADGES[key].desc;
    list.appendChild(li);
  });
